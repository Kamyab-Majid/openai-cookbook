...
class DataCheck:
    def __init__(
        ...
        self.config = self.resolve_config(
            config_path.replace("config.json", "env.json"), json.loads(config_content)
        )
        ...
        try:
            self.output_columns = self.config[src_system]["sources"][self.file_name][
                "dq_output_columns"
            ]
        except Exception:
    ...
    def read_s3_file(self, file_path):
        ...
        try:
            ...
            raise FileNotFoundError(f"File cannot be found in S3 given path '{file_path}'") from e
    ...
    def data_type_check(self, input_col):
        ...
        if dtype_key == "DateType":
            ...
            self.source_df = self.source_df.withColumn(
                input_col + " schema", f.to_date(f.col(input_col), date_format)
            )
        else:
            dtype = self.schema_dict[dtype_key]()
            self.source_df = self.source_df.withColumn(
                input_col + " schema", f.col(input_col).cast(dtype)
            )
        ...
        self.add_error_col(
            error_msg=type_error_msg, condition=dtype_cond, error_col_name=input_col + " type_check"
        )
    ...
    def null_check(self, input_col):
        ...
        self.add_error_col(
            ...
            error_col_name=input_col + " null_check",
        )
    ...
    def conditional_check(self, input_col: str) -> None:
        ...
        for cond_ind, condition_columns in enumerate(multiple_conditional_list):
            current_additional_cond_value = self.rule_df.loc[
                input_col, "conditional_column_value"
            ].split(";")[cond_ind]
            current_conditional_valuelist = self.rule_df.loc[
                input_col, "conditional_valuelist"
            ].split(";")[cond_ind]
    ...
    def range_check(self, input_col):
        ...
        range_error_cond = range_error_cond & (
        ...
        self.add_error_col(
            error_msg=range_error_msg,
            condition=range_error_cond,
            error_col_name=input_col + " range_check",
        )
    ...
    def file_check(self, input_col):
        ...
        if type(reference_columns_str) == str:
            additional_columns_list = (
                reference_columns_str.replace(", ", ",").replace(" ,", ",").split(",")
            )
        ...
        for col_ind, ref_col in enumerate(reference_columns):
            ...
            try:
                ...
                self.source_df = self.source_df.withColumn(
                    source_col, f.trim(f.upper(f.col(source_col)))
                )
                self.source_df = self.source_df.withColumn(
                    source_col, f.regexp_replace(source_col, "-", " ")
                )
                ...
                self.sns_message.append(
                    f"Column {ref_col} is not found in the RDW file needed for DQ check"
                )
                return None, None
        ...
        file_cond = (f.col(join_col).isNull()) & (
            (f.col(input_col).isNotNull()) | (f.col(input_col) != "")
        )
        file_error_msg = f"{file_check_type}_FAIL: Column [{source_df_columns_list}] did not pass the {file_check_type}."
    ...
    def category_check(self, input_col: str) -> None:
        ...
        if valuelist_type[:2] == "__":
            ...
            category_error_msg = (
                f"category_FAIL: Column [{input_col}] accepted values are ({category_list}])"
            )
        self.add_error_col(
    ...
    def duplicate_cond_syntax(self, input_col):
        self.source_df: DataFrame = self.source_df.join(
            broadcast(
                self.source_df.groupBy(input_col).agg((f.count("*")).alias("Duplicate_indicator"))
            ),
            on=input_col,
            how="inner",
        )
    ...
    def main_pipeline(self):
        ...
        columns_to_check_dict[self.null_check] = self.rule_df[
            (self.rule_df["nullable"]).isna()
        ].index
        ...
        self.error_df = self.source_df.select(
            self.output_columns[0], f.explode("final").alias("Error")
